/**
 * Temporary Flam visual QA harness.
 */
...
const pages = [
  { path: "/", label: "home" },
  { path: "/flam-shore-excursions", label: "excursions" },
  { path: "/excursions/flam-railway", label: "railway" },
  { path: "/flam-port-guide", label: "port-guide" },
  { path: "/one-day-in-flam", label: "one-day" },
  { path: "/flam-cruise-ship-schedule", label: "schedule-guide" },
  { path: "/ship-schedule", label: "schedule-hub" },
  { path: "/ship-schedule/july-2026", label: "month-busy" },
  { path: "/ship-schedule/december-2027", label: "month-light" },
  ...
  for (const vp of viewports) {
    ...
    for (const route of pages) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        const sibling = /Lysefjord|Pulpit Rock|Bryggen|Nidaros|Bakklandet/i.test(
          body.innerText || "",
        );
      ...
      if (
        metrics.emailLinks.some((h) =>
          (h || "").includes("hello@flamshoreexcursions.com"),
        )
      ) {
        note(
          "FAIL",
      ...
      if (metrics.sibling && !route.path.includes("flam-port-guide")) {
    ...
    if (vp.width === 390 || vp.width === 1440) {
      await page.goto(`${base}/ship-schedule/july-2026`, {
        waitUntil: "networkidle2",
        timeout: 60000,
      });
