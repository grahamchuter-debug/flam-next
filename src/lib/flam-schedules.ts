import schedulePayload from "@/data/flam-cruise-schedules.generated.json";

export type FlamScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
type SchedulePayload = {
  ...
  rows: FlamScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const flamScheduleIntegrity = payload.integrity;
export const flamScheduleMeta = {
  port: payload.port,
...
function toEntry(row: FlamScheduleRow): FlamScheduleEntry {
  return {
...
export function getAllFlamRows(): FlamScheduleRow[] {
  return payload.rows;
}

export function getFlamEntries(): FlamScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getFlamEntriesForMonthKey(monthKey: string): FlamScheduleEntry[] {
  return payload.rows
...
export function getFlamMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getFlamMonthSummaries() {
